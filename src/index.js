...
class View extends React.Component {
  ...
  actions = {
    ...
    setUser: user => this.setState({ user: user, visibleRooms: user.rooms }),
    ...
    createRoom: options => {
      this.state.user.createRoom(options).then(this.actions.joinRoom).then(()=>{this.setState({visibleRooms: this.state.user.rooms})})
    },
    onChange: options => {
      let value = options.target.value.toString();
      const tempUser = this.state.user;
      let visibleRooms = []
      if(value.trim() === ' '){
        visibleRooms = tempUser.rooms
      }else{
        visibleRooms = tempUser.rooms.filter((room) => room.name.includes(value))
      }
  ...
  render() {
    ...
    const { createRoom, onChange, createConvo, removeUserFromRoom } = this.actions
    ...
    return (
      <main>
        <aside data-open={sidebarOpen}>
          ...
          <RoomList
            user={user}
            rooms={this.state.visibleRooms}
            messages={messages}
            typing={typing}
            current={room}
            actions={this.actions}
          />
          {user.id && <CreateRoomForm submit={createRoom} onChange={onChange}/>}
        </aside>
        <section>
